...
public class ListPembeli24 {
    ...
    void addFirst(Pembeli24 item24) {
        if (isEmpty()) {
    ...
    void add(Pembeli24 item24, int index) throws Exception {
        if (isEmpty()) {
            addFirst(item24);
    ...
    void print() {
        if (!isEmpty()) {
            Node24 tmp = head24;
            while (tmp != null) {
                System.out.print(tmp.pembeli24.kodeAntri24 + "\t" + tmp.pembeli24.namaPembeli24 + "\t\t" + tmp.pembeli24.NoHp24 + "\n");
                tmp = tmp.next24;
            }
            //System.out.println("\nBerhasil diisi");
        } else {
    ...
    Pembeli24 getFirst() throws Exception {
        if (isEmpty()) {
            throw new Exception("Linked List kosong");
        }
        return head24.pembeli24;
    }

    Pembeli24 getLast() throws Exception {
        if (isEmpty()) {
            throw new Exception("Linked List kosong");
        }
        Node24 tmp = head24;
        ...
        return tmp.pembeli24;
    }

    Pembeli24 get(int index) throws Exception {
        if (isEmpty() || index >= size24) {
            throw new Exception("Nilai indeks di luar batas");
        }
        Node24 tmp = head24;
